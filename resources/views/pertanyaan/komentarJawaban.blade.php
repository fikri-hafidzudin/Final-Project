...
<form role="form" action="{{ route('komentarJawaban.store') }}" method="POST">
                          ...
                          @forelse ($jawab->komentar as $key => $komen)
                                <div style="display: flex;">
                                    ...
                                    <a href="{{ route('komentarJawaban.edit', $komen->id) }}" class="btn btn-warning btn-sm mr-1 mt-1">edit</a>
                                    <form action="{{ route('komentarJawaban.destroy', $komen->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <input type="submit" value="delete" class="btn btn-danger btn-sm mr-1 mt-1">
                                    </form>
                                    </div>
                                </div>
                            @empty
